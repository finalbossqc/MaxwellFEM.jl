...
function sample_boundary_2D(N::Int64, Lx::Float64, Ly::Float64; rng=Random.default_rng())
    ...
    boundary_points = boundary_points ∪ [Meshes.Point(point.coords.x.val, Ly) for point in sample_domain_1D(Int(sqrt(N)), Lx)]
    boundary_points = boundary_points ∪ [Meshes.Point(0, point.coords.x.val) for point in sample_domain_1D(Int(sqrt(N)), Lx)]
    boundary_points = boundary_points ∪ [Meshes.Point(Lx, point.coords.x.val) for point in sample_domain_1D(Int(sqrt(N)), Lx)]
